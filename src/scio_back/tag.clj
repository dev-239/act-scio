...
(defrecord Alias [name aliases population extra])
...
(defn parse-ti-alias-line
  ...
  [ln]
  (let [[actor alias-list] (str/split ln #"\s*:\s*" 2)
        aliases (if alias-list
                  (str/split alias-list #"\s*,\s*")
                  [])]
...
(defn parse-location-alias-line
  ...
  [ln]
  (let [[_ name _ alternatnames _ _ _ _ country-code _ _ _ _ _ population & rest] (str/split ln #"\t")
        aliases (filter
                  #(not= % (str/upper-case %)) ;; remove all uppercase only aliases
                  (str/split alternatnames #","))]
    (map->Alias {:name name
                 :aliases aliases
                 :extra country-code
...
(defn parse-country-info-line
  "parse-country-info-line parses the format of a line in countryInfo.txt from geonames"
  [ln]
  (let [[iso _ _ _ country _ _ population & rest] (str/split ln #"\t")]
    (map->Alias {:name country
                 :aliases [iso]
                 :population (Integer. population)})))

(defn map-file
  "map-file read a file on record by line format providing a parser-function returning an Alias"
  [fn file-name]
...
(defn alias->index-map-v2
  "Take an map, [i, alias] and update an index map only if the new alias has a greater 'population'"
  [mymap [i alias]]
...
(defn create-index-v2
  "Take a list of aliases and create an index map giving priority to large cities and places."
  [aliases]
...
(defn alias->regex
  ...
  (let [wordlist (cons (:name alias) (:aliases alias))
        proto-pattern (str/join "|" wordlist)]
...
(defn possible-sector?
  ...
  [tag]
  (let [lcase-tag (str/lower-case tag)
    ...
    (->> (zip (repeat lcase-tag) sector-postfix)
         (filter #(apply str/includes? %))
         seq)))

(defn sector-stem
  "Find the stem of a sector (remove last word; organization, industry etc.) and also any article from the front"
  [s]
